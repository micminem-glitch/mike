"use client";

import React, { useEffect, useState } from 'react';
import { supabase } from '../lib/supabase';

interface Transaction {
  id: string;
  user_id: string;
  type: string;
  recipient_account: string;
  amount: number;
  status: string;
  created_at: string;
}

export default function AdminDashboard() {
  const [allTransactions, setAllTransactions] = useState<Transaction[]>([]);
  const [uniqueUsers, setUniqueUsers] = useState<string[]>([]);
  const [loading, setLoading] = useState(true);
  
  // Balance Modification States
  const [targetUserId, setTargetUserId] = useState('');
  const [adjustmentAmount, setAdjustmentAmount] = useState('');
  const [adjustmentType, setAdjustmentType] = useState<'Online Deposit' | 'Admin Deduction'>('Online Deposit');
  
  // Custom Profile Override States
  const [customLimit, setCustomLimit] = useState('2,000,000.00');
  const [customLoan, setCustomLoan] = useState('0.00');
  const [customStatus, setCustomStatus] = useState('Active');
  
  const [actionLoading, setActionLoading] = useState(false);
  const [adminMessage, setAdminMessage] = useState<{ type: 'success' | 'error', text: string } | null>(null);

  // Load EVERY transaction and map user registries
  async function loadMasterLedger() {
    setLoading(true);
    const { data, error } = await supabase
      .from('transactions')
      .select('*')
      .order('created_at', { ascending: false });

    if (!error && data) {
      setAllTransactions(data);
      
      const usersSet = new Set<string>();
      data.forEach(tx => {
        if (tx.user_id) usersSet.add(tx.user_id);
      });
      setUniqueUsers(Array.from(usersSet));
    }
    setLoading(false);
  }

  useEffect(() => {
    loadMasterLedger();
  }, []);

  // Handler to safely add or subtract user balances
  const handleBalanceAdjustment = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!targetUserId || !adjustmentAmount) return;

    setActionLoading(true);
    setAdminMessage(null);

    // If it's a deduction, the amount logic keeps track of it as a negative entry
    const finalAmount = parseFloat(adjustmentAmount);

    const { error } = await supabase
      .from('transactions')
      .insert([
        {
          user_id: targetUserId,
          amount: finalAmount,
          type: adjustmentType,
          recipient_account: adjustmentType === 'Online Deposit' ? 'ADMIN CREDIT INJECTION' : 'ADMIN DEBIT OVERRIDE',
          status: 'Settled'
        }
      ]);

    setActionLoading(false);

    if (error) {
      setAdminMessage({ type: 'error', text: `Adjustment failed: ${error.message}` });
    } else {
      setAdminMessage({ type: 'success', text: `Successfully updated balance parameters for Node ID!` });
      setAdjustmentAmount('');
      loadMasterLedger();
    }
  };

  // Profile override simulations
  const handleSaveProfileOverrides = (e: React.FormEvent) => {
    e.preventDefault();
    if (!targetUserId) {
      setAdminMessage({ type: 'error', text: 'Please select an active target node first.' });
      return;
    }
    setAdminMessage({ 
      type: 'success', 
      text: `Saved configuration profile metrics for ${targetUserId.slice(0,8)}... (Limit: $${customLimit}, Status: ${customStatus})` 
    });
  };

  return (
    <div className="space-y-8 w-full min-h-screen bg-slate-950 p-6 text-white">
      {/* HEADER BANNER */}
      <div className="bg-gradient-to-r from-red-950/40 to-slate-900 border border-red-900/40 p-6 rounded-2xl">
        <h2 className="text-xl font-black tracking-wide uppercase text-red-400">Hexafox Executive Master Terminal</h2>
        <p className="text-slate-400 text-sm">System-wide transactional overrides, account metric manipulation, and user ledger configurations.</p>
      </div>

      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
        
        {/* LEFT PANEL: CHOOSE ACCOUNTS & EDIT STATS */}
        <div className="lg:col-span-4 space-y-6">
          
          {/* USER SELECTION PANEL */}
          <div className="bg-[#0b132b] p-6 rounded-2xl border border-slate-800 shadow-xl">
            <h3 className="font-bold text-sm mb-1 text-slate-200">Registered System Nodes</h3>
            <p className="text-xs text-slate-500 mb-4">Select an account profile ID to control:</p>
            
            {loading ? (
              <p className="text-xs text-slate-500 animate-pulse">Scanning identity registry...</p>
            ) : (
              <div className="space-y-2 max-h-[180px] overflow-y-auto pr-1">
                {uniqueUsers.map((uid) => (
                  <button
                    key={uid}
                    type="button"
                    onClick={() => setTargetUserId(uid)}
                    className={`w-full text-left font-mono text-xs p-2.5 rounded-xl border transition-all flex justify-between items-center ${
                      targetUserId === uid 
                        ? 'bg-blue-600/20 border-blue-500 text-blue-400' 
                        : 'bg-black/30 border-slate-800 hover:border-slate-700 text-slate-300'
                    }`}
                  >
                    <span>ID: {uid.slice(0, 12)}...</span>
                    <span className="text-[10px] uppercase font-bold text-slate-400 bg-slate-900 px-2 py-0.5 rounded">Target</span>
                  </button>
                ))}
              </div>
            )}
          </div>

          {/* BALANCE INJECTION / DEDUCTION LOGIC */}
          <div className="bg-[#0b132b] p-6 rounded-2xl border border-slate-800 shadow-xl">
            <h3 className="font-bold text-sm mb-4 text-slate-200">Modify Account Balance</h3>
            
            <form onSubmit={handleBalanceAdjustment} className="space-y-4">
              <div className="grid grid-cols-2 gap-2 bg-black/40 p-1 rounded-xl border border-slate-800">
                <button
                  type="button"
                  onClick={() => setAdjustmentType('Online Deposit')}
                  className={`py-2 text-xs font-bold rounded-lg transition-all ${adjustmentType === 'Online Deposit' ? 'bg-emerald-600 text-white' : 'text-slate-400 hover:text-white'}`}
                >
                  ➕ Inject ($+)
                </button>
                <button
                  type="button"
                  onClick={() => setAdjustmentType('Admin Deduction')}
                  className={`py-2 text-xs font-bold rounded-lg transition-all ${adjustmentType === 'Admin Deduction' ? 'bg-red-600 text-white' : 'text-slate-400 hover:text-white'}`}
                >
                  ➖ Minus ($-)
                </button>
              </div>

              <div>
                <label className="block text-[10px] font-semibold text-slate-400 uppercase mb-1">Target Account String</label>
                <input 
                  type="text"
                  value={targetUserId}
                  onChange={(e) => setTargetUserId(e.target.value)}
                  className="w-full bg-[#111a36] border border-slate-700/50 rounded-xl p-3 text-xs text-white font-mono placeholder-slate-500"
                  placeholder="Select a Node ID above..."
                  required
                />
              </div>
              <div>
                <label className="block text-[10px] font-semibold text-slate-400 uppercase mb-1">Adjustment Value ($)</label>
                <input 
                  type="number"
                  value={adjustmentAmount}
                  onChange={(e) => setAdjustmentAmount(e.target.value)}
                  className="w-full bg-[#111a36] border border-slate-700/50 rounded-xl p-3 text-xs text-white placeholder-slate-500"
                  placeholder="0.00"
                  required
                />
              </div>
              <button 
                type="submit"
                disabled={actionLoading}
                className={`w-full font-bold py-3 rounded-xl text-xs uppercase tracking-wider transition-all ${adjustmentType === 'Online Deposit' ? 'bg-emerald-600 hover:bg-emerald-700' : 'bg-red-600 hover:bg-red-700'}`}
              >
                {actionLoading ? 'Executing Delta Override...' : 'Execute Balance Shift'}
              </button>
            </form>
          </div>

          {/* EDIT ACCOUNT PROFILE CONFIGURATION */}
          <div className="bg-[#0b132b] p-6 rounded-2xl border border-slate-800 shadow-xl">
            <h3 className="font-bold text-sm mb-4 text-slate-200">Edit Node Properties</h3>
            
            <form onSubmit={handleSaveProfileOverrides} className="space-y-4">
              <div>
                <label className="block text-[10px] font-semibold text-slate-400 uppercase mb-1">Account Custom Limit</label>
                <input 
                  type="text"
                  value={customLimit}
                  onChange={(e) => setCustomLimit(e.target.value)}
                  className="w-full bg-[#111a36] border border-slate-700/50 rounded-xl p-3 text-xs text-white font-mono"
                />
              </div>
              <div>
                <label className="block text-[10px] font-semibold text-slate-400 uppercase mb-1">Assigned Loan Balance</label>
                <input 
                  type="text"
                  value={customLoan}
                  onChange={(e) => setCustomLoan(e.target.value)}
                  className="w-full bg-[#111a36] border border-slate-700/50 rounded-xl p-3 text-xs text-white font-mono"
                />
              </div>
              <div>
                <label className="block text-[10px] font-semibold text-slate-400 uppercase mb-1">Node Status Level</label>
                <select
                  value={customStatus}
                  onChange={(e) => setCustomStatus(e.target.value)}
                  className="w-full bg-[#111a36] border border-slate-700/50 rounded-xl p-3 text-xs text-white outline-none"
                >
                  <option value="Active">🟢 Active / Enforced Tunnel</option>
                  <option value="Suspended">🟡 Verification Hold</option>
                  <option value="Terminated">🔴 Terminated / Locked Node</option>
                </select>
              </div>
              <button 
                type="submit"
                className="w-full bg-blue-600 hover:bg-blue-700 text-white font-bold py-3 rounded-xl text-xs uppercase tracking-wider transition-all"
              >
                Save Account Updates
              </button>
            </form>
          </div>

        </div>

        {/* RIGHT PANEL: FULL TRANSACTION REAL-TIME FEED */}
        <div className="lg:col-span-8 bg-[#0b132b] p-6 rounded-2xl border border-slate-800 shadow-xl flex flex-col justify-between">
          <div>
            <h3 className="font-bold text-sm mb-4 text-slate-200">Global Ledger Stream Trace</h3>
            {adminMessage && (
              <div className={`p-3 rounded-xl text-xs mb-4 ${adminMessage.type === 'success' ? 'bg-emerald-500/10 text-emerald-400 border border-emerald-500/20' : 'bg-red-500/10 text-red-400 border border-red-500/20'}`}>
                {adminMessage.text}
              </div>
            )}
            
            {loading ? (
              <p className="text-sm text-slate-500 animate-pulse">Running full-network array query...</p>
            ) : (
              <div className="overflow-x-auto max-h-[640px] overflow-y-auto">
                <table className="w-full text-left text-xs text-slate-300">
                  <thead className="bg-[#111a36] text-slate-400 font-semibold sticky top-0">
                    <tr>
                      <th className="p-3">User Account ID</th>
                      <th className="p-3">Type Classification</th>
                      <th className="p-3">Delta Value</th>
                      <th className="p-3">Timestamp</th>
                    </tr>
                  </thead>
                  <tbody className="divide-y divide-slate-800/60">
                    {allTransactions.map((tx) => {
                      const renderDisplayId = tx.user_id && typeof tx.user_id === 'string'
                        ? `${tx.user_id.slice(0, 8)}...`
                        : 'Legacy Rec';

                      const isDeposit = tx.type === 'Online Deposit';

                      return (
                        <tr key={tx.id} className="hover:bg-slate-900/40">
                          <td className="p-3 font-mono text-slate-400 bg-slate-950/20 select-all" title={tx.user_id || ''}>
                            {renderDisplayId}
                          </td>
                          <td className="p-3 font-medium">
                            <span className={isDeposit ? 'text-emerald-400' : 'text-slate-300'}>
                              {tx.type}
                            </span>
                          </td>
                          <td className={`p-3 font-bold ${isDeposit ? 'text-emerald-400' : 'text-red-400'}`}>
                            {isDeposit ? '+' : '-'}${Number(tx.amount).toLocaleString(undefined, { minimumFractionDigits: 2 })}
                          </td>
                          <td className="p-3 text-slate-500">{new Date(tx.created_at).toLocaleDateString()}</td>
                        </tr>
                      );
                    })}
                  </tbody>
                </table>
              </div>
            )}
          </div>
        </div>

      </div>
    </div>
  );
}